 ...
 */
 class ListsSuite {
  ...
  @Test def `sum of negatives`: Unit = {
    Assert.assertEquals(-103, sum(List(-1,-2,-100)))
  }

  @Test def `sum to zero`: Unit = {
    Assert.assertEquals(0, sum(List(-1,0,1)))
  }

  @Test def `sum of zero`: Unit = {
    Assert.assertEquals(0, sum(List(0,0)))
  }

  @Test def `sum of repeated`: Unit = {
    Assert.assertEquals(18, sum(List(-1,-1,0,0,1,1,2,2,3,3,4,4)))
  }
  ...
  @Test(expected = classOf[NoSuchElementException])
    def `max of empty`: Unit = {
      max(List())
  }

  @Test def `max of negatives`: Unit = {
    Assert.assertEquals(-1, max(List(-1,-2,-100)))
  }

  @Test def `max of mixed sign`: Unit = {
    Assert.assertEquals(1, max(List(-1,0,1)))
  }

  @Test def `max of zero`: Unit = {
    Assert.assertEquals(0, max(List(0,0)))
  }

  @Test def `max of repeated`: Unit = {
    Assert.assertEquals(4, max(List(-1,-1,0,0,1,1,2,2,3,3,4,4)))
  }
